...
public class ApplicationCommandsAuthor {
    ...
    public void showAuthorById(String id) {
        ...
        ioService.outputString(String.valueOf(author.findById(id)));
    }

    @ShellMethod(key = {"an", "author-name"}, value = "One author by name: <command> [name]")
    public void showOneAuthor(String name) {
        ioService.outputString("Запрос в базу авторов по NAME=" + name);
        if (author.findByName(name).isEmpty()) {
            ioService.outputString("Автора с именем " + name + " не существует!!!");
        } else {
            author.findByName(name).forEach(author -> ioService.outputString(String.valueOf(author)));
        }
    }
    ...
    public void showAllAuthor() {
        ...
        author.findAll().forEach(author -> ioService.outputString(String.valueOf(author)));
    }
    ...
    public void showInsertAuthor(String authorName) {
        ...
        ioService.outputString(String.valueOf(author.save(new Author(authorName))));
    }
    ...
    public void showUpdateAuthor(String id, String newNameAuthor) {
        ...
        ioService.outputString(String.valueOf(author.save(new Author(id, newNameAuthor))));
    }
}
